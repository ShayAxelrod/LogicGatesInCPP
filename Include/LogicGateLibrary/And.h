...
class And : public MultipleInput<T>{
public:
    template<typename First, typename ... Others>
    And(First* input1, Others* ... others) : MultipleInput<T>(){
        this->insertInputsToVector< First, Others...>(input1, others...);
    }

    T operation(){
        T result = std::accumulate(this->input->begin(), this->input->end(), resultOfFirstElement(), [] (T l1, ILogic<T>* l2){
